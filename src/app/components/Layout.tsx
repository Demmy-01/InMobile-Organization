...
export function Layout() {
  ...
  // Derive initials and display info from profile
          ...
          <nav className="flex-1 px-3 py-5 overflow-y-auto space-y-6">
            {navItems.map((section) => (
              <div key={section.section}>
                <p className="text-xs font-bold tracking-widest mb-2 px-2" style={{ color: "rgba(255,255,255,0.35)" }}>
                  {section.section}
                </p>
                  ...
                  {section.items.map((item) => {
                    ...
                    if ((item as any).isAction) {
                      return (
                        <button
                          key={item.label}
                          onClick={() => { setShowPostModal(true); closeSidebar(); }}
          ...
          <div className="px-4 py-4 relative" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setShowUserMenu((v) => !v)}>
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: "#2d6bcf", overflow: "hidden" }}>
                {logoUrl ? (
                  <img src={logoUrl} alt={orgName} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} />
                ) : (
                  initials
                )}
              </div>
          ...
          <header className="flex items-center justify-between px-4 sm:px-6 py-3 flex-shrink-0 gap-3" style={{ background: "#0d1b3e", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-white font-bold text-base sm:text-xl leading-tight truncate">
                  {currentPage.title}
                </h1>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)" }}>
